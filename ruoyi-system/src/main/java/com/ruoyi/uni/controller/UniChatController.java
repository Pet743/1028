...
public class UniChatController {
    ...
    @Autowired
    private IAlseChatConversationService chatConversationService;
    ...
    public AjaxResult getOrCreateConversation(@Validated @RequestBody GetConversationRequestDTO requestDTO) {
        ...
        if (targetUserId == null) {
            return AjaxResult.error("目标用户ID不能为空");
        }

        try {
            ConversationResponseDTO conversation = chatService.getOrCreateConversation(userId, targetUserId);
            return AjaxResult.success(conversation);
        } catch (Exception e) {
            log.error("获取或创建会话失败: {}", e.getMessage(), e);
    ...
    public AjaxResult getConversationList(@Validated @RequestBody MessageListRequestDTO requestDTO) {
        // 调用服务获取会话列表
        List<ConversationListResponseDTO> conversationList = chatConversationService.getConversationList(requestDTO.getUserId());
        return AjaxResult.success(conversationList);
    }
